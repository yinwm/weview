...
- `index refresh`：普通维护入口。没有索引时从 0 构建；上次中断时从 checkpoint / watermark 续跑；已有完整索引时优先用 `session/session.db` 找活跃会话，只刷新这些 chat 的新增 rows。daemon 正在运行时优先请求 daemon 的内部 `refresh_index`。
...
输出字段包括 `status`、`query_mode`、`schema_version`、`source_db_count`、`indexed_rows`、`source_rows`、`covered_chats`、`job_state`、`lag_seconds`、`session_lag_seconds`、`lag_policy`、`reason`、`refresh_mode`、`active_chats_checked`、`active_chats_refreshed`、`reconcile_sources_pending`、`reconcile_tables_checked` 和 `path`。状态值：
...
性能注意：索引 ready、准实时且覆盖全部选中会话时，`timeline` 会用普通 B-tree 索引按全局时间顺序取候选 row refs，再回源 cache 读取完整消息。全局 source DB stale 但全部选中 chat 已按 `session.db` 追平时也可以走索引；只要有一个选中 chat 未追平，就会回落到按会话 fan-out 扫描本地 `message/message_*.db` 分片，合并后再应用全局 `--limit`。仍然建议使用小时间窗口和明确筛选，例如 `--username wxid_xxx --date today` 或 `--kind chatroom --query AI --date today`；避免对 `--kind all` 使用宽时间范围。
...
- 更细粒度的增量索引：当前 `index refresh` 已经优先使用 `session.db` 做活跃会话热刷新，并用小批量 reconcile 兜底；后续可以继续优化 reconcile 调度和变更估计。
